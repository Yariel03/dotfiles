...
# 1. Función interna que genera el catálogo coloreado con íconos visuales y ejecuta fzf
show_cheatsheet() {
    ...
    cat << EOF | fzf --ansi \
                     --reverse \
                     --header=$'  👑 GUIA DE ATAJOS Y COMANDOS - HYPRLAND 👑\n  [Escriba para filtrar] • [Esc / q / Enter para salir]\n' \
                     --prompt="  🔍 Buscar: " \
                     --pointer="▶ " \
...
${C_CAT}🚀 APLICACIONES${NC}   ${C_KEY}SUPER + G${NC}                   ${C_DESC}⏺️  Script de grabacion de pantalla (grabar.sh)${NC}
...
${C_CAT}🪟 VENTANAS${NC}       ${C_KEY}SUPER + Clic Izquierdo (LMB)${NC} ${C_DESC}🖱️  Arrastrar y mover ventana flotante con raton${NC}
${C_CAT}🪟 VENTANAS${NC}       ${C_KEY}SUPER + Clic Derecho (RMB)${NC}   ${C_DESC}📐 Redimensionar tamano de ventana con raton${NC}

${C_CAT}📸 CAPTURAS${NC}       ${C_KEY}SUPER + C  o  PrtSc${NC}         ${C_DESC}✂️  Captura de region seleccionada (Hyprshot)${NC}
...
${C_CAT}📸 CAPTURAS${NC}       ${C_KEY}SUPER + ALT + C  o  CTRL+PrtSc${NC} ${C_DESC}🖥️  Capturar pantalla completa${NC}

${C_CAT}🖥️  ESCRITORIOS${NC}    ${C_KEY}SUPER + 1 .. 9${NC}              ${C_DESC}🔢 Cambiar al espacio de trabajo 1 al 9${NC}
${C_CAT}🖥️  ESCRITORIOS${NC}    ${C_KEY}SUPER + SHIFT + 1 .. 9${NC}      ${C_DESC}📦 Mover ventana activa al espacio 1 al 9${NC}
${C_CAT}🖥️  ESCRITORIOS${NC}    ${C_KEY}SUPER + ALT + Izq / Der${NC}     ${C_DESC}↔️  Navegar entre escritorios activos${NC}
${C_CAT}🖥️  ESCRITORIOS${NC}    ${C_KEY}SUPER + S${NC}                   ${C_DESC}⭐ Alternar espacio de trabajo especial (Scratchpad)${NC}
${C_CAT}🖥️  ESCRITORIOS${NC}    ${C_KEY}SUPER + SHIFT + S${NC}           ${C_DESC}📥 Enviar ventana al espacio especial (Scratchpad)${NC}
${C_CAT}🖥️  ESCRITORIOS${NC}    ${C_KEY}SUPER + Rueda Raton (Scroll)${NC} ${C_DESC}🖱️  Desplazarse entre escritorios de trabajo${NC}

${C_CAT}⚙️  SISTEMA${NC}        ${C_KEY}SUPER + H${NC}                   ${C_DESC}⌨️  Ver esta guia de atajos y comandos${NC}
${C_CAT}⚙️  SISTEMA${NC}        ${C_KEY}SUPER + L${NC}                   ${C_DESC}🔒 Bloquear pantalla (Hyprlock)${NC}
${C_CAT}⚙️  SISTEMA${NC}        ${C_KEY}SUPER + N${NC}                   ${C_DESC}🔔 Abrir/Cerrar centro de notificaciones (SwayNC)${NC}
${C_CAT}⚙️  SISTEMA${NC}        ${C_KEY}SUPER + M${NC}                   ${C_DESC}🚪 Salir de Hyprland (Exit session)${NC}

${C_CAT}🎵 MULTIMEDIA${NC}     ${C_KEY}Volumen +/- / Silenciar${NC}     ${C_DESC}🔊 Subir/bajar volumen y mutear microfono (SwayOSD)${NC}
${C_CAT}🎵 MULTIMEDIA${NC}     ${C_KEY}Play / Pause / Next / Prev${NC}  ${C_DESC}⏯️  Control de reproduccion (YouTube, Spotify, etc.)${NC}
EOF
}
